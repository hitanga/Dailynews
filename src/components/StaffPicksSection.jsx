...
export default function FunFactsSection({ blogs }) {
  ...
  const sidePicks = blogs.slice(1, 5);
    ...
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 border-t border-gray-100">
      ...
      <div className="mb-8">
        <span className="block text-[11px] font-bold tracking-[0.22em] text-[#f84560] uppercase mb-1.5 flex items-center gap-1.5">
          <span>💡</span>
          <span>DID YOU KNOW? • CURATED TRIVIA & STORIES</span>
        </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        {mainPick && (
          <div className="lg:col-span-7 group">
            <Link to={`/blog/${mainPick.id}`} className="block relative aspect-[16/10] bg-gray-100 overflow-hidden mb-4 rounded-sm">
              <img
              ...
              <div className="absolute top-3 left-3 bg-amber-500 text-white font-bold text-[10px] tracking-wider uppercase px-2.5 py-1 rounded shadow-sm">
                Fun Fact
              </div>
            </Link>
        ...
        <div className="lg:col-span-5 space-y-6">
          {sidePicks.map((pick) => (
            <article key={pick.id} className="flex gap-4 items-start group pb-6 border-b border-gray-100 last:border-0 last:pb-0">
              <Link to={`/blog/${pick.id}`} className="w-24 sm:w-28 h-20 sm:h-24 bg-gray-100 shrink-0 overflow-hidden relative block rounded-sm">
                <img
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[9px] sm:text-[10px] font-bold tracking-[0.15em] text-amber-600 uppercase mb-1">
